...
describe("MySkillsPage", () => {
  ...
  it("renders skill name and row-level management actions", async () => {
    ...
    expect(await findByLabelText("Test Skill linked tools")).toBeInTheDocument();
  ...
  it("opens a lightweight confirmation popover before bulk sync", async () => {
    ...
    expect(
      await findByText("This will sync the currently visible enabled Skills to all enabled tools."),
    ).toBeInTheDocument();
  ...
  it("keeps sync-all switch off and shows failure details when all tools fail during sync-all", async () => {
    ...
    vi.mocked(getSkills).mockResolvedValue([mockSkill]);
    vi.mocked(getTools).mockResolvedValue(mockTools.map((tool) => ({ ...tool, is_enabled: true })));
    vi.mocked(getSkillLinks).mockResolvedValueOnce([]).mockResolvedValueOnce([]);
    ...
    expect(
      await findByText("Claude (No write permission), Codex (No write permission)"),
    ).toBeInTheDocument();
    await waitFor(() => {
      expect(syncAll).toBeEnabled();
    });
