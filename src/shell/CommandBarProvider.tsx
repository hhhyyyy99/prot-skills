...
export function CommandBarProvider({ children }: { children: ReactNode }) {
  ...
  const commands = useMemo<CommandItem[]>(() => {
    ...
    const skills: CommandItem[] = scanLocalRef.current
      ? [{ id: 'scan-local', label: t('command.scanLocal'), group: 'Skills', perform: () => scanLocalRef.current?.() }]
      : [];
    const theme: CommandItem[] = [
      { id: 'set-theme-system', label: t('command.theme.system'), group: 'Theme', keywords: ['auto'], perform: () => setPreference('system') },
      { id: 'set-theme-light', label: t('command.theme.light'), group: 'Theme', perform: () => setPreference('light') },
      { id: 'set-theme-dark', label: t('command.theme.dark'), group: 'Theme', perform: () => setPreference('dark') },
    ];
