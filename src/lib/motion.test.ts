...
import { MOTION, DURATIONS, EASINGS } from './motion';
...
describe('motion constants', () => {
  ...
  it('all MOTION values are var() references', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...Object.keys(MOTION) as (keyof typeof MOTION)[]),
        (key) => {
  ...
  it('all EASINGS values are var() references', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...Object.keys(EASINGS) as (keyof typeof EASINGS)[]),
        (key) => {
