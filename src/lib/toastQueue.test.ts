...
import { toastReducer, type ToastState, type ToastPayload } from './toastQueue';
...
describe('toastReducer', () => {
  ...
  it('queue length never exceeds 3', () => {
    fc.assert(
      fc.property(
        fc.array(toastArb, { minLength: 1, maxLength: 10 }),
        (toasts) => {
  ...
  it('maintains FIFO order', () => {
    fc.assert(
      fc.property(
        fc.array(fc.nat(100).map(n => makeToast(`t${n}`)), { minLength: 1, maxLength: 6 }),
        (toasts) => {
          let state = emptyState;
          for (const t of toasts) {
            state = toastReducer(state, { type: 'enqueue', payload: t });
          ...
          return state.queue.length === expected.length &&
