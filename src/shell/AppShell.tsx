...
const PRIMARY_NAV_ITEMS: readonly PrimaryNavItem[] = [
  {
    id: "my-skills",
    label: "nav.mySkills",
    icon: Folder as PrimaryNavItem["icon"],
    shortcut: "⌘1",
  },
  ...
  { id: "migrate", label: "nav.migrate", icon: Search as PrimaryNavItem["icon"], shortcut: "⌘3" },
  {
    id: "settings",
    label: "nav.settings",
    icon: Settings as PrimaryNavItem["icon"],
    shortcut: "⌘4",
  },
];
...
export function AppShell() {
  ...
  useEffect(() => {
    setNavigateHandler(setActivePage);
  }, [setNavigateHandler]);

  const navCollapsed = bp !== "regular";
  // eslint-disable-next-line oxc/no-map-spread -- small constant array, intentional copy-on-write
  const navItems = useMemo(
    () => PRIMARY_NAV_ITEMS.map((item) => ({ ...item, label: t(item.label) })),
    [t],
  );
